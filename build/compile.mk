...
compile-all: deps-only
	...
	@for b in $(BINS); do \
		for os in $(COMPILE_OS); do \
			...
			CGO_ENABLED=1 GOOS=$$os $(GO) build -ldflags=$(LDFLAGS) -o $(BUILD_DIR)/$$os/$$b $$BUILD_FILES ; \
		done \
	done
...
compile-only: deps-only
	...
	@for b in $(BINS); do \
		...
		BUILD_FILES=`find $(SRCDIR)/cmd/$$b -type f -name "*.go"` ; \
		CGO_ENABLED=1 GOOS=$(GOOS) $(GO) build -ldflags=$(LDFLAGS) -o $(BUILD_DIR)/$(GOOS)/$$b $$BUILD_FILES ; \
	done
